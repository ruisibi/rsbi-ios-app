...
//    self.title = @"";
    ...
    if ([self.iscollect integerValue] == 1) {
        // 右上角按钮
        UIImage *imgRight = [UIImage imageNamed:@"btn_stored"];
        ...
        [rightBtn addTarget:self action:@selector(clickDel) forControlEvents:UIControlEventTouchUpInside];
        
        [self.navigationItem setRightBarButtonItem:[[UIBarButtonItem alloc]initWithCustomView:rightBtn]];
    }else{
        UIImage *imgRight = [UIImage imageNamed:@"btn_store"];
        UIButton *rightBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        rightBtn.frame = CGRectMake(0, 0, imgRight.size.width, imgRight.size.height);
        [rightBtn setBackgroundImage:imgRight forState:UIControlStateNormal];
...
//        NSLog(@"删除返回 ----- %@",operation.responseString);
        // 判断token是否可用
        ...
        if (errStr != nil && [errStr isEqualToString:self.rid]) {
            UIImage *imgRight = [UIImage imageNamed:@"btn_store"];
            UIButton *rightBtn = [UIButton buttonWithType:UIButtonTypeCustom];
            rightBtn.frame = CGRectMake(0, 0, imgRight.size.width, imgRight.size.height);
            [rightBtn setBackgroundImage:imgRight forState:UIControlStateNormal];
...
//    manager.requestSerializer = [AFJSONRequestSerializer serializer];
    ...
    NSDictionary     *parameters = @{@"token": [Common shareInstence].token
                                     ,@"rid":self.rid};
    NSString *urlString = WEB_SERVICE_COLLECTADD([Common GetServiceHost]);
    
    [manager POST:urlString parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
        NSDictionary *adict = responseObject;
        if ([[adict objectForKey:@"result"] integerValue] == 1) {
            [[RGHudModular getRGHud] showAutoHudWithMessageDefault:@"收藏成功"];
            UIImage *imgRight = [UIImage imageNamed:@"btn_stored"];
            UIButton *rightBtn = [UIButton buttonWithType:UIButtonTypeCustom];
            rightBtn.frame = CGRectMake(0, 0, imgRight.size.width, imgRight.size.height);
            [rightBtn setBackgroundImage:imgRight forState:UIControlStateNormal];
